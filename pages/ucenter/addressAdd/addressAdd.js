...
Page({
    ...
    getAddressDetail() {
        let _this = this;
        util.request(api.AddressDetail, { id: _this.data.addressId }).then(function(res) {
            if (res.data.errno === 0) {
                _this.setData({
    ...
    setRegionDoneStatus() {
        let _this = this;
        let doneStatus = _this.data.selectRegionList.every(item => {
            return item.id != 0;
        });
    ...
    selectRegionType(event) {
        ...
        let selectRegionList = _this.data.selectRegionList;
    ...
    selectRegion(event) {
        ...
        _this.setData({
            regionList: _this.data.regionList.map(item => {

                //标记已选择的
                if (_this.data.regionType == item.type && _this.data.selectRegionList[_this.data.regionType - 1].id == item.id) {
                    item.selected = true;
                } else {
                    item.selected = false;
                }
    ...
    getRegionList(regionId) {
        let _this = this;
        let regionType = _this.data.regionType;
            ...
            if (res.data.errno === 0) {
                _this.setData({
                  regionList: res.data.data.map(item => {
                        ...
                        //标记已选择的
                        if (regionType == item.type && _this.data.selectRegionList[regionType - 1].id == item.id) {
                            item.selected = true;
                        } else {
                            item.selected = false;
                        }
